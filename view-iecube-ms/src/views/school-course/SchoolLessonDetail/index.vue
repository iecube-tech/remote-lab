        ...
        <el-col :span="6">
          ...
          <el-card>
            ...
            <div v-for="(item, index) in data.attachmentList" :key="'attachment' + index" class="course-attachment">
              <a target="_blank" :href="'local-resource/' + item.key" :title="item.filename">{{ item.filename }}</a>
            </div>
          </el-card>
        </el-col>
      </el-row>
...
export default {
  ...
  mounted() {
    this.id = parseInt(this.$route.params.schoolLessonId)
    this.teacherCourseId = parseInt(this.$route.params.schoolCourseId)
    this.fetchData()
  },
  methods: {
    fetchData() {
