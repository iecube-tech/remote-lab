        ...
        <el-col :span="6">
          ...
          <el-card>
            ...
            <div v-for="(item, index) in data.attachmentList" :key="'attachment' + index" class="course-attachment">
              <a target="_blank" :href="'local-resource/' + item.key" :title="item.originFilename">{{ item.filename }}</a>
            </div>
          </el-card>
        </el-col>
      </el-row>
...
export default {
  ...
  data() {
    return {
      id: undefined,
      teacherCourseId: undefined,
      data: {}
    }
  },
  mounted() {
    this.id = parseInt(this.$route.params.teacherLessonId)
    this.teacherCourseId = parseInt(this.$route.params.teacherCourseId)
    this.fetchData()
  },
  methods: {
    fetchData() {
